...
public class BaseService<TBllEntity, TDalEntity, TDalRepository> 
    : BaseService<TBllEntity, TDalEntity, TDalRepository, Guid>, IBaseService<TBllEntity>
    where TBllEntity : class, IDomainId
    where TDalEntity : class, IDomainId
    where TDalRepository : class, IBaseRepository<TDalEntity>
{
    public BaseService(IBaseUow serviceUow, TDalRepository serviceRepository, IMapper<TBllEntity, TDalEntity, Guid> mapper) 
    ...
    where TKey : IEquatable<TKey>
{
    ...
    public BaseService(IBaseUow serviceUow, TDalRepository serviceRepository,
        IMapper<TBllEntity, TDalEntity, TKey> mapper)
    {
    ...
    {
        var entity = ServiceRepository.Find(id, userId);
        if (entity != null) ServiceRepository.Remove(entity, userId);
    }

    public virtual async Task RemoveAsync(TKey id, TKey? userId = default)
    {
        var entity = await ServiceRepository.FindAsync(id, userId);
        if (entity != null) await ServiceRepository.RemoveAsync(id, userId);
    }
}